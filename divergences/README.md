...
- [11 — OVER: incremental accumulators, not a window exec](11-over-incremental-vs-window-exec.md) — running `OVER` folds DataFusion accumulators per key (matching Flink) rather than running a DataFusion window plan over buffered rows like Arroyo; the unbounded frame makes incremental strictly better. Future general window functions (`ROW_NUMBER`/`LAG`) *will* delegate to a window exec, as Arroyo does.
