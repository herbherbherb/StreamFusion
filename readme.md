...
| `OVER` aggregate (running) | Yes | Event-time `OVER ([PARTITION BY k] ORDER BY rt)` with the default `RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW` frame: one or more `SUM`/`MIN`/`MAX`/`COUNT` over the same bigint/int/double value column, optionally partitioned by one or more bigint/int/string keys, emitting each input row with the running aggregate(s) appended. Each row is held until the watermark passes its rowtime, then folded per partition in rowtime order (ties within a key share the value). Bounded/`ROWS` frames, `AVG`, and proctime fall back to the host. |
