...
| Filter (`WHERE`), with column projection | Conjunctions of comparisons | A filter whose predicate is a comparison — or an `AND` of comparisons — of a numeric column against a numeric literal (`>` `>=` `<` `<=` `<>`, either order) or a string column against a string literal (`<>`), with an optional projection that selects/reorders input columns (`SELECT a, c FROM t WHERE …`); `BETWEEN`/ranges expand to comparisons and route. The whole row is carried through Arrow, so every input column must be a type the [whole-row converter](src/main/java/io/github/jordepic/streamfusion/operator/RowDataArrowConverter.java) handles (the primitives, boolean, string). Equality (`=`) on the filtered column falls back (the column is constant-folded into the projection — pending general expression projection); projections with computed expressions or constants, disjunctions, non-comparison terms, and other column types fall back. A projection with no filter is left to Flink (no native gain). |
